function [ Flipmean_aug,namelistcell_afteraug,gtbox_afteraug,gtlabel_afteraug,set_afteraug,imgsize_afteraug,boxesall_afteraug,flip_afteraug] = ImgFlipAug( path,namelistcell,gtbox,gtlabel,set,imgsize,w_resize,h_resize,boxesall,imgdata_aug)
...
parfor i=1:batch_num
    [ Flipmean_aug{i},namelistcell_batch{i},gtbox_batch{i},gtlabel_batch{i},set_batch{i},imgsize_batch{i},boxesall_batch{i} ,flip_batch{i}] = ImgFlipAug_batch( path,namelistcell(batch_idx_cell{i}),gtbox(batch_idx_cell{i}),gtlabel(batch_idx_cell{i}),set(batch_idx_cell{i}),imgsize(batch_idx_cell{i},:),w_resize,h_resize,boxesall(batch_idx_cell{i}) );
...
for i=1:batch_num
    ...
    flip_afteraug=cat(1,flip_afteraug,flip_batch{i});
end
...
%     total_imgnum=total_imgnum+hsvmean_aug_batch{i}.num;
    meanR=(meanR*total_imgnum+Flipmean_aug{i}.rmean*Flipmean_aug{i}.num)...
        /(total_imgnum+Flipmean_aug{i}.num);
    meanG=(meanG*total_imgnum+Flipmean_aug{i}.gmean*Flipmean_aug{i}.num)...
        /(total_imgnum+Flipmean_aug{i}.num);
    meanB=(meanB*total_imgnum+Flipmean_aug{i}.bmean*Flipmean_aug{i}.num)...
        /(total_imgnum+Flipmean_aug{i}.num);
    total_imgnum=total_imgnum+Flipmean_aug{i}.num;
...
Flipmean_aug=struct('num',total_imgnum,'rmean',meanR,'gmean',meanG...
    ,'bmean',meanB);
end
