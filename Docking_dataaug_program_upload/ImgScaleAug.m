function [ Scalemean_aug,namelistcell_afteraug,gtbox_afteraug,gtlabel_afteraug,set_afteraug,imgsize_afteraug,boxesall_afteraug ] = ImgScaleAug( path,namelistcell,gtbox,gtlabel,set,imgsize,w_resize,h_resize,scalefactor,boxesall,imgdata_aug)
...
Scalemean_aug_batch=cell(1,batch_num);
...
parfor i=1:batch_num
    [ Scalemean_aug_batch{i},namelistcell_batch{i},gtbox_batch{i},gtlabel_batch{i},set_batch{i},imgsize_batch{i},boxesall_batch{i} ] =  ImgScaleAug_batch( path,namelistcell(batch_idx_cell{i}),gtbox(batch_idx_cell{i}),gtlabel(batch_idx_cell{i}),set(batch_idx_cell{i}),imgsize(batch_idx_cell{i},:),w_resize,h_resize,scalefactor,boxesall(batch_idx_cell{i}));
...
%     total_imgnum=total_imgnum+hsvmean_aug_batch{i}.num;
    meanR=(meanR*total_imgnum+Scalemean_aug_batch{i}.rmean*Scalemean_aug_batch{i}.num)...
        /(total_imgnum+Scalemean_aug_batch{i}.num);
    meanG=(meanG*total_imgnum+Scalemean_aug_batch{i}.gmean*Scalemean_aug_batch{i}.num)...
        /(total_imgnum+Scalemean_aug_batch{i}.num);
    meanB=(meanB*total_imgnum+Scalemean_aug_batch{i}.bmean*Scalemean_aug_batch{i}.num)...
        /(total_imgnum+Scalemean_aug_batch{i}.num);
    total_imgnum=total_imgnum+Scalemean_aug_batch{i}.num;
...
Scalemean_aug=struct('num',total_imgnum,'rmean',meanR,'gmean',meanG...
    ,'bmean',meanB);
