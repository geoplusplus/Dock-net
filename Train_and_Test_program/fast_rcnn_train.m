...
%addpath(fullfile(vl_rootnn,'examples','myfast_rcnn_161109','bbox_functions'));
addpath(fullfile('bbox_functions'));
%addpath(fullfile(vl_rootnn,'examples','myfast_rcnn_161109','datasets'));
addpath(fullfile('datasets'));

opts.dataDir   = fullfile('../', 'data') ;
opts.sswDir    = fullfile('../', 'data', 'SSW');
opts.expDir    = fullfile('../', 'data') ;
opts.imdbPath  = fullfile(opts.expDir, 'docking_imdb.mat');
opts.modelPath = fullfile(opts.dataDir, 'models', ...
  'mylenet80x64.mat') ;%dock-net-c model
...
%opts.train.learningRate = 1e-3 / 64 * [ones(1,70) 0.1*ones(1,30)];
opts.train.learningRate = 1e-3 / 64 * [ones(1,40) 0.1*ones(1,10)];%determin the training epochs
...
net = fast_rcnn_init(...
  ...
  'modelPath',opts.modelPath,'imdbpath',opts.imdbPath);
...
imdb = cnn_setup_data_voc07_ssw(...
     ...
     'sswDir', opts.sswDir, ...
     'addFlipped', true, ...
     'useDifficult', true,'imdbpath',opts.imdbPath) ;
...
[im,rois,labels,btargets] = My_fast_rcnn_train_get_batch(images,imdb,...
  batch, opts);
...
net.params(net.getParamIndex(blayer.params{1})).value = ...
    bsxfun(@times,filters,...
    reshape([boxStds(:)' zeros(1,4,'single')]',...
    [1 1 1 4*(numel(net.meta.classes.name)+1)]));
