...
%function net = fast_rcnn_init(varargin,imdbpath)
...
nCls=2;
%lastfccellnum=1000;%cell num of last fully connected layer 
myROIpoolsize=[2 2];
lastfc=find(cellfun(@(a) strcmp(a.name, 'layer13'), net.layers)==1);
net.layers{lastfc}.name = 'predcls';
%net.layers{lastfc}.weights{1} = 0.01 * randn(1,1,myROIpoolsize(1)*myROIpoolsize(2)*size(net.layers{lastfc}.weights{1},3),nCls,'single');
...
%last fc before classification and regression 
% net.addLayer('lastfcbeforeclassreg',dagnn.Conv('size',[1 1 size(myROIpoolsize,1)*size(myROIpoolsize,2)...
%     *size(net.params(mylastrelu).value,3) lastfccellnum],'hasBias', true), ...
%    'xPR','lastfcbeforeclassreg',{'predbboxf','predbboxb'});
...
net.addLayer('predbbox',dagnn.Conv('size',[1 1 myROIpoolsize(1)*myROIpoolsize(2)*depth_Roipooloutput 4*nCls],'hasBias', true), ...
 ...
 %net.params(end-1).value = 0.001 * randn(1,1,myROIpoolsize(1)*myROIpoolsize(2)*depth_Roipooloutput,4*nCls,'single');
